(async () => {
  ...
  const stateKey = "__GT_RISING_ROWS_V18__";
  const rowsStorageKey = "GT_RISING_ROWS_V18";
  const checkpointStorageKey = "GT_RISING_CHECKPOINT_V18";
  const popupId = "gt_rising_popup_v18";
  ...
  let rows = window[stateKey] || readJsonStorage(rowsStorageKey, []);
  ...
  if (!box) {
    ...
    box.innerHTML = [
      ...
      '<label style="display:flex;align-items:center;gap:4px;color:#5f6368">geo <input data-x="geo-count" type="number" value="1" min="-1" step="1" style="width:48px;padding:3px 5px;border:1px solid #dadce0;border-radius:4px"></label>',
      '<input data-x="geo-list" value="GLOBAL,US,DE,BR,IN,CN,MX" title="Comma-separated geos. Use GLOBAL for global." style="width:170px;padding:3px 5px;border:1px solid #dadce0;border-radius:4px">',
      '<label style="display:flex;align-items:center;gap:4px;color:#5f6368">cat <input data-x="category-count" type="number" value="1" min="-1" step="1" style="width:48px;padding:3px 5px;border:1px solid #dadce0;border-radius:4px"></label>',
      '<button data-x="collect">collect</button>',
      '<button data-x="continue">continue</button>',
  ...
  const continueButton = box.querySelector('[data-x="continue"]');
  const geoCountInput = box.querySelector('[data-x="geo-count"]');
  const geoListInput = box.querySelector('[data-x="geo-list"]');
  const categoryCountInput = box.querySelector('[data-x="category-count"]');
  const isolatePopupEvent = (event) => {
  ...
  [
    ...
    box.addEventListener(eventName, isolatePopupEvent);
  });
  ...
  function checkpoint() {
    return readJsonStorage(checkpointStorageKey, null);
  }

  function writeCheckpoint(value) {
    writeJsonStorage(checkpointStorageKey, {
      ...value,
  ...
  function persistRows() {
    ...
    window[stateKey] = rows;
    writeJsonStorage(rowsStorageKey, rows);
  }
  ...
  function normalizeGeo(value) {
    ...
    return geoAliases[raw] || geoAliases[lower] || raw.toUpperCase();
  }

  function geoCount() {
    const value = Number(geoCountInput?.value || 1);
    if (value === -1) return -1;
    if (!Number.isFinite(value) || value < 1) return 1;
    return Math.floor(value);
  }
  ...
  function geoList() {
    const defaults = ["GLOBAL", "US", "DE", "BR", "IN", "CN", "MX"];
    const parsed = String(geoListInput?.value || "")
      .split(/[,，\s]+/)
      .map(normalizeGeo)
      .filter(Boolean);
    ...
    for (const geo of list) {
      if (!unique.includes(geo)) unique.push(geo);
    }
    return unique;
  }

  function geoSequence(current, count) {
    const list = geoList();
    const geo = normalizeGeo(current || "GLOBAL") || "GLOBAL";
    if (count === -1) return list;
    return [geo, ...list.filter((item) => item !== geo)].slice(0, count);
  }

  function initializeGeoList() {
    if (!geoListInput) return;
    const geo = currentGeo();
    const list = geoList();
    if (!list.includes(geo)) geoListInput.value = [geo, ...list].join(",");
  }
  ...
  function buildTargetUrl(baseUrl, geo, cat) {
    const url = new URL(baseUrl);
    const normalizedGeo = normalizeGeo(geo || "GLOBAL") || "GLOBAL";
    if (normalizedGeo === "GLOBAL") url.searchParams.delete("geo");
    else url.searchParams.set("geo", normalizedGeo);
  ...
  function taskList(geos, cats) {
    const tasks = [];
    for (const geo of geos) {
      for (const cat of cats) tasks.push({ geo, cat });
    }
    return tasks;
  }

  async function collectTaskWithRetries(targetWindow, baseUrl, task, label, useCollectorWindow) {
    let lastError = null;
    for (let attempt = 1; attempt <= 3; attempt += 1) {
      try {
        if (useCollectorWindow) {
          targetWindow.location.href = buildTargetUrl(baseUrl, task.geo, task.cat);
        }
        setStatus(`${label}: loading${attempt > 1 ? ` retry ${attempt}/3` : ""}`);
        return await collectSingleCategory(targetWindow, label);
      } catch (error) {
        lastError = error;
        if (isRateLimitError(error)) throw error;
        setStatus(`${label}: retry ${attempt}/3 after ${error.message || error}`);
  ...
  async function collectRows(resume = false) {
    if (collecting) return;
    collecting = true;
    collectButton.disabled = true;
    if (geoCountInput) geoCountInput.disabled = true;
    if (geoListInput) geoListInput.disabled = true;
    if (categoryCountInput) categoryCountInput.disabled = true;
    const beforeCollectCount = uniqueRows(rows).length;

    try {
      const saved = resume ? checkpoint() : null;
      const current = meta(window);
      const geos = saved?.geos || geoSequence(current.geo, geoCount());
      const cats = saved?.cats || categorySequence(current.cat, categoryCount());
      const baseUrl = saved?.baseUrl || window.location.href;
      const tasks = taskList(geos, cats);
      let startTaskIndex = Math.max(0, Math.min(Number(saved?.taskIndex || 0), tasks.length));
      if (!resume) clearCheckpoint();
      if (resume && !saved) {
        setStatus("no checkpoint to continue");
        return;
      }
      if (resume && startTaskIndex >= tasks.length) {
        clearCheckpoint();
        setStatus("checkpoint already complete");
        return;
      }

      const useCollectorWindow = tasks.length > 1 ||
        tasks[startTaskIndex]?.geo !== current.geo ||
        tasks[startTaskIndex]?.cat !== current.cat;
      ...
      for (let index = startTaskIndex; index < tasks.length; index += 1) {
        ...
        const label = `geo ${task.geo} cat ${task.cat} (${index + 1}/${tasks.length})`;
        writeCheckpoint({ baseUrl, geos, cats, taskIndex: index, task });

        try {
          await collectTaskWithRetries(targetWindow, baseUrl, task, label, useCollectorWindow);
          persistRows();
          writeCheckpoint({ baseUrl, geos, cats, taskIndex: index + 1, task });
        } catch (error) {
          failures.push(`${task.geo}/${task.cat}: ${error.message || error}`);
          persistRows();
          ...
          stoppedReason = `stopped at ${label}`;
          setStatus(`${stoppedReason}; click continue after network recovers`);
          break;
        }
        if (index < tasks.length - 1) await sleep(1500);
      }

      const suffix = failures.length ? `, failed tasks ${failures.length}` : "";
      if (stoppedReason) {
      ...
      if (geoCountInput) geoCountInput.disabled = false;
      if (geoListInput) geoListInput.disabled = false;
      if (categoryCountInput) categoryCountInput.disabled = false;
    }
  }

  box.querySelector('[data-x="collect"]').onclick = () => collectRows(false);
  box.querySelector('[data-x="continue"]').onclick = () => collectRows(true);
