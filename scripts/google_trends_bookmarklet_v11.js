(async () => {
  ...
  const stateKey = "__GT_RISING_ROWS_V11__";
  const popupId = "gt_rising_popup_v11";
  ...
  function render(debug = "") {
    ...
    bodyEl.innerHTML = [
      ...
      '<tbody>',
      currentRows.map((row) => `<tr><td>${escapeHtml(row.geo)}</td><td>${escapeHtml(row.cat)}</td><td>${escapeHtml(row.date)}</td><td>${escapeHtml(row.query)}</td><td>${escapeHtml(row.translation)}</td><td>${escapeHtml(row.change)}</td></tr>`).join(""),
      '</tbody></table>',
      ...
      for (let index = 0; index < cats.length; index += 1) {
        ...
        try {
          ...
          if (isRateLimitError(error)) {
            setStatus(`stopped by rate limit at ${label}; total ${uniqueRows(rows).length}`);
            break;
          }
