(async () => {
  ...
  const stateKey = "__GT_RISING_ROWS_V16__";
  const popupId = "gt_rising_popup_v16";
  ...
  async function collectRows() {
    ...
    try {
      ...
      const geos = geoSequence(current.geo, geoCount());
      const cats = categorySequence(current.cat, categoryCount());
      const baseUrl = window.location.href;
      const useCollectorWindow = geos.length > 1 || cats.length > 1 || geos[0] !== current.geo || cats[0] !== current.cat;
      ...
      let stoppedReason = "";
      let taskIndex = 0;
      const taskTotal = geos.length * cats.length;
      for (let geoIndex = 0; geoIndex < geos.length; geoIndex += 1) {
        const geo = geos[geoIndex];
        for (let catIndex = 0; catIndex < cats.length; catIndex += 1) {
          const cat = cats[catIndex];
          taskIndex += 1;
          const label = `geo ${geo} cat ${cat} (${taskIndex}/${taskTotal})`;
          try {
            if (useCollectorWindow) {
              targetWindow.location.href = buildTargetUrl(baseUrl, geo, cat);
            }
            setStatus(`${label}: loading`);
            await collectSingleCategory(targetWindow, label);
          } catch (error) {
            failures.push(`${geo}/${cat}: ${error.message || error}`);
          ...
          if (taskIndex < taskTotal) await sleep(1500);
        }
