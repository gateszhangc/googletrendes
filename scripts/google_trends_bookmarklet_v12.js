(async () => {
  ...
  const storageKey = "GT_RISING_ROWS_V12";
  const popupId = "gt_rising_popup_v12";
  ...
  function writeStoredRows(value) {
    localStorage.setItem(storageKey, JSON.stringify(value));
  }

  let rows = readStoredRows();
  ...
  async function collectRows() {
    ...
    try {
      ...
      setStatus(`done, added ${uniqueRows(rows).length - beforeCollectCount}, total ${uniqueRows(rows).length}`);
      render();
    } catch (error) {
      setStatus("failed");
  ...
  box.querySelector('[data-x="save"]').onclick = () => {
    const blob = new Blob([makeTsv(rows)], { type: "text/tab-separated-values;charset=utf-8" });
